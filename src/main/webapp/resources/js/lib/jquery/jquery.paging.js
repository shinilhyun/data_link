...
!function($){
	...
	//default properties.
	var a=/a/i,defs={
		item:'a',next:'<img src="' + _contextPath + 'resources/images/egovframework/com/cmm/next_btn.png" />',prev:'<img src="' + _contextPath + 'resources/images/egovframework/com/cmm/prev_btn.png" />',format:'{0}',
		...
		,href:'#{0}',event:true,first:'<img src="' + _contextPath + 'resources/images/egovframework/com/cmm/pprev_btn.png" />',last:'<img src="' + _contextPath + 'resources/images/egovframework/com/cmm/nnext_btn.png" />'
	},format=function(str){
		var arg=arguments;
	...
	$.fn.paging=function(op){
		$(this).each(function(){
			...
			if(op.current>op.length){
				if(op.first!==false && (parseInt(op.start/op.length)>1)) make(op,1,op.sideClass,op.first);//first button
			...
			if(op.current/op.length<op.max/op.length){
				...
				if(op.last!==false && (parseInt((op.max-op.start)/op.length)>1)) make(op,op.max,op.sideClass,op.last);//last button
			}
			this.__JQ_PAGING=op;
		});
	};
